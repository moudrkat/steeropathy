...
A reader of this experiment's LinkedIn post — Garret Sutherland — showed up in the
comments with a series of null hypotheses. Each one was run the same day, live, at
his specified config. The point of this repo is the **mechanism**, not any claim
about emotions — but knowing what the wire actually carries is mechanism work, so
the audit is permanent bench material now (`python -m steeropathy.transmit --null`,
details in [transmit](transmit.md)):
