...
| **[zombie](experiments/zombie.md)** | a bias outbreak: one mind is bitten so its neutrality goes silent and it turns into a confident advocate, and the still-neutral minds fight back by reading the fading neutrality off each other's J-space — never a word between them. Vector-agnostic (`--strain`): the infection can be a behaviour or a concept. Plus the honest limits — J-space ≈ logit lens for early detection, and a concept only spreads if the model doesn't already do it |
