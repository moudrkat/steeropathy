...
| **[resonance](experiments/resonance.md)** | four minds read and pay to push each other; a hunt for equilibrium that demolished eight of my own theories |
...
Start brainscope first (any recent build with the `/capture` endpoint), then
steeropathy:

```bash
# 1. brainscope — hosts the model. Add --jlens <lens.pt> --traces <dir> for the
#    J-space channel (resonance uses it; the rest run fine without).
...
Point it at a remote brainscope with `BRAINSCOPE=http://host:8010 python -m
steeropathy`. Each experiment's own run commands live on its page above; the web UI
runs all four live, one round per click, and `#replay` animates the last saved run
without a GPU.
...
- A concept vector is a property of the **contrast you chose**, not of the model.
  ...
  and you've measured *sadness*. Same model, same sentences, opposite sign.
