...
| **[secondhand](experiments/secondhand.md)** | mind A gets a wish and dreams a [brave-new-world](https://github.com/moudrkat/brave-new-world) page; its activations are pushed into mind B, which dreams the world of a wish it never heard. The page is the readout, field by field, against A's own poem as the text channel. Sixty wishes, three seeds, two controls: the poem carries everything the vector does and more; what the vector adds, a shuffled vector adds too. `worldof`: what a direction looks like when the model draws it, twelve worlds a direction, placebo each |
| **[soundof](experiments/soundof.md)** | the same directions through a second instrument: eight bars in ABC notation under the vector, tempo, key and meter as the readout, placebo each. Built; first live run on the 4B in the queue |
